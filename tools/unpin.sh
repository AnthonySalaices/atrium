...
# Prefers the restore record atriumd leaves on the window (@atrium_prev);
# falls back to "unset the manual size" when there is none.
set -u
for s in $(tmux list-sessions -F '#{session_name}' 2>/dev/null); do
  rec=$(tmux show-options -w -v -t "$s" @atrium_prev 2>/dev/null)
  if [ -n "$rec" ]; then
    ...
    tmux set-option -w -u -t "$s" @atrium_prev
